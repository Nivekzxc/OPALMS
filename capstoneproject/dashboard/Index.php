			...
			<div class="wrapper employee_list clearfix">
				<div class="menu-search">
		 		...
		 		<?php if(isset($_GET['Searching'])){
		 			$Search = $_GET['Search'];
		 			$fltr = $_GET['filter'];
		 			header("location:searchemp.php?Name=$Search&By=$fltr");
		 			} ?>
		 	</div><br><hr>
				<?php
					$getemp = mysql_query("SELECT * FROM tbl_employee ORDER BY EmployeeID ASC");
					$getempcount = mysql_num_rows($getemp);
				...
				<ul class="emp_list">
					...
					<div id="displayempList">
						<?php
							if($getempcount >= 1 ){
								while($fetch = mysql_fetch_array($getemp)){
